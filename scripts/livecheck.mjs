...
// change can be measured on the real page before it is released. <check.js> is the body of an
...
ws.addEventListener("message", e => {
  ...
  const u = m.params.request.url, marker = "website-css@" + tag + "/";
  if (cdnSha) {
...
await send("Fetch.enable", { patterns: [{ urlPattern: "*website-css@" + tag + "/*" }] });
