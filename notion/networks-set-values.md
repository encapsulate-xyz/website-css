...
| Lido DVT | Simple DVT node operator #43 (SSV cluster "Arid Anubis") | The Lido and SSV.network rows combined into one (the user, 2026-09-24); the SSV.network row is archived. **Stake at** = https://stake.lido.fi. Stakers pay Lido's 10%; the rate is stETH's 7-day APR. **Validators run** = 500 (module 2, operator 43: 500 deposited, 0 exited). Our fee is inside the 10%: module 8% / treasury 2% (StakingRouter), and the cluster's split sends 2/7 to Lido's Agent and 5/49 to each of the seven operators (0xSplits `0xcddc0b19f2011933b7c20df7ebb378a0b941a187`, created in tx `0x30c5a234…58dd1`), ≈0.82% of rewards each. SSV operator 924's own fee is 0; 30-day performance 99.43% (SSV API, 2026-09-24) — not stored. Our **second** Simple DVT cluster, operator #48 "Lido x SSV: Mysterious Manta" (160 keys, all 160 exited), was ours and shut down on purpose (the user, 2026-09-24): it counts toward nothing — not Validators run, not Slashing events |
